...
func (p *PostProductRepo) Products() ([]entity.Product, error) {
	rows, err := p.conn.Query("SELECT * FROM sproduct;")
	if err != nil {
...
func (p *PostProductRepo) SoldP(id int) ([]entity.Product, error){
	rows, err := p.conn.Query("SELECT  * from sproduct WHERE userid = $1", id)
	if err != nil {
		return nil, errors.New("Could not query")
	}
	defer rows.Close()
...
func (p *PostProductRepo) Product(id int) (entity.Product, error) {
	rows := p.conn.QueryRow("SELECT * from sproduct WHERE id = $1", id)
...
func (p *PostProductRepo) UpdateP(pro entity.Product) error {
	_, err := p.conn.Exec("UPDATE sproduct SET name = $1, ptype=$2, price=$3, description=$4,Image=$5,userid=$6 WHERE id = $6", pro.Name, pro.Ptype, pro.Price, pro.Description, pro.Image, pro.ID, pro.UserID)
	if err != nil {
...
func (p *PostProductRepo) StoreP(pro entity.Product) error {
	_, err := p.conn.Exec("INSERT INTO sproduct (name,ptype,price,description,Image,userid,createdat)"+"values($1,$2,$3,$4,$5,$6,current_timestamp)", pro.Name, pro.Ptype, pro.Price, pro.Description, pro.Image, pro.UserID)
	if err != nil {
		panic(err)
...
func (p *PostProductRepo) DeleteP(id int) error {
	_, err := p.conn.Exec("DELETE FROM sproduct WHERE id=$1", id)
	if err != nil {
...
func (p *PostProductRepo) SearchProduct(prod string) ([]entity.Product, error) {
	rows, err := p.conn.Query("SELECT * FROM sproduct WHERE name LIKE $1", "%"+prod+"%")
